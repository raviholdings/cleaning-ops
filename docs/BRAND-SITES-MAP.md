...
|---|---|---|---|---|---|---|
| `dream` | 드림컴뚜러 | dreamcome.kr | 070-7106-5225 | **506** `yuzjplo3322` | 평면 · **글형식** | 5,046 |
| `thunder` | 썬더배관 | thunderdrain.kr | 070-7106-5226 | 502 `dynfbs403` | 평면 · 카드형(동 글은 글형식) | 5,063 |
| `mole` | 비버배관 | beaverpipe.kr | 070-7106-5227 | 503 `zfbbt34` | 평면 · 카드형(동 글은 글형식) | 5,046 |
...
- 합계 **22,112장**. 2026-09-03 에 드림·썬더·비버에 동 단위 글을 4,760장씩
  붙였다 (286/303/286 -> 5,046/5,063/5,046).
- 장수는 `naver_project_domains.page_count` 에 있고 관리자 화면이 이걸 읽는다.
  `deploy-brand-sites.mjs` 가 배포할 때마다 맞춘다 (`--no-db` 로 끌 수 있다).
...
- **수집요청 한도는 계정이 아니라 사이트에 붙는다.** 도메인이 다섯뿐이라 하루
  ...
  전량 한 바퀴에 dream 101일 · thunder 102일 · mole 101일 · ssak 67일 · dosa 73일.
  동 글이 붙기 전 드림·썬더·비버는 6일이면 끝났다. 대량 색인은 IndexNow 쪽이 답이다.
