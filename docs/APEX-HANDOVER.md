...
- [x] **IndexNow 제출** (2026-08-26) — 서치어드바이저 등록·계정 없이 발견 경로를
      여는 방법. 루트의 `<key>.txt` 하나가 소유 증명이다. **도메인마다 키가 달라야
      한다** (네이버 가이드). 키는 `data/apex/indexnow-keys.json`, 제출은
      `scripts/submit-apex-indexnow.mjs`. 루트 10개 전부 200 Success.
      `/form/` 은 제출하지 않았다(운영자 지정).
      확인은 `scripts/check-apex-yeti-visits.mjs` — 기준선 apex 전부 0회
