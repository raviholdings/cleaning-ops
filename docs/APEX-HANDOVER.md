...
- [ ] **배포** — `tmp/apex/` 에만 있다. 서버로 안 올렸다
- [ ] **심볼릭 링크 4개 제거** — `naoheg.com` `one-qfast.com` `oneshot-sewer.com`
      `pipe-oneshot.com` 의 웹루트가 서브도메인을 가리키는 심볼릭 링크다
      (2026-08-05 14:17 KST 수동 생성). 배포 전에 걷어내야 한다.
      **삭제 전 대상·건수를 운영자에게 보고할 것**
