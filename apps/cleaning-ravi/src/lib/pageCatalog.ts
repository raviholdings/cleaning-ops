 ...
 *
 * 31 인 이유는 재고가 딱 거기까지이기 때문이다. 확장 영역에 남는 슬롯은
 * 조합수 1,312,884 에서 기준선 1,000,000 을 뺀 312,884 개뿐이고, 이 값을
 * 사이트 10,000 개로 나누면 31.28 이다. 그래서 31 이 전 사이트가 확장을
 * 받을 수 있는 최대치다.
 *
 * 예전에는 50 이었다. 그러면 앞쪽 6,258 개 사이트가 남은 슬롯을 다 먹어
 * 6,259 번째부터는 101 번 한 장도 못 만들고 catalogEntry 가 던진다.
 * (2026-08-17 에 계정 51~103 배포가 실제로 여기서 죽었다.)
 *
 * 이 값을 바꾸면 101 번 이후 페이지의 지역·키워드 배정이 전부 밀린다.
 * 기준선(1~100)은 다른 경로로 계산하므로 영향을 받지 않는다.
 */
export const EXPANSION_INCREMENT = 31;
