/* 비버배관 스크립트 — 시도/시군구 연동 · 접수 폼 · 동의 전문 */
...
(function(){
  ...
  document.addEventListener("submit",async function(e){
    ...
    var payload={
      ...
      message:(function(){
        /* 브리프 3 이 요구한 쉬운 질문(거주 형태·문제 공간·발생 시점)을 받는다.
           lead_submissions 에 컬럼을 늘리지 않으려고 문의내용 뒤에 붙여 보낸다.
           표에 새 컬럼을 만드는 건 운영자 확인이 필요한 일이라 여기서 하지 않는다. */
        var base=String(d.get("message")||"").trim();
        var extra=[];
        var sels=form.querySelectorAll("[data-extra]");
        for(var i=0;i<sels.length;i++){
          var v=String(sels[i].value||"").trim();
          if(v) extra.push(sels[i].getAttribute("data-extra")+": "+v);
        }
