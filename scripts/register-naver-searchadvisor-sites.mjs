...
async function ensureAccountIp(account) {
   ...
   *
   * HaiIP 는 유동이라 예전 IP 가 다시 안 나올 수 있다. DB 에는 그 옛 IP 가
   * 남아 있어서 다시 실행해도 같은 IP 를 요구하고 또 실패한다.
   * 수집요청 러너와 소유확인 스크립트가 하는 것과 같게 맞춘다.
   *
   * 조건 하나 — 현재 IP 를 다른 계정이 쓰고 있으면 안 된다. 계정 여러 개가
   * 한 IP 를 쓰면 네이버가 묶어서 볼 수 있고, 그걸 막는 게 이 구조의 근간이다.
   */
...
function rebindSessionIp(accountId, ip) {
  ...
  try {
    ...
    execFileSync(process.execPath, [
      ...
      '--note', `register rebind ${new Date().toISOString()}`,
    ], { stdio: 'pipe', cwd: projectRoot });
